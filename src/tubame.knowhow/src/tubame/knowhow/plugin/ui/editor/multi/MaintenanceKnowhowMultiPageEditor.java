 ...
 * editor).<br/>
 */
public class MaintenanceKnowhowMultiPageEditor extends FormEditor implements
        EditorDirty {

    /** Logger */
    private static final Logger LOGGER = LoggerFactory
            .getLogger(MaintenanceKnowhowMultiPageEditor.class);
    ...
    protected void addPages() {
        ...
        documentationFormPage = new DocumentationFormPage(this, null,
                ResourceUtil.documentation);
        checkItemFieldsPage = new CheckItemFieldsPage(this, null,
                ResourceUtil.checkItem);
        try {
            ...
            JbmException
                    .outputExceptionLog(
                            ...
                            MessagePropertiesUtil
                                    .getMessage(MessagePropertiesUtil.FAIL_CREATE_KNOWHOW_EDITOR));
    ...
    protected void initializePageSwitching() {
        ...
        try {
            ...
            JbmException.outputExceptionLog(LOGGER, e, MessagePropertiesUtil
                    .getMessage(MessagePropertiesUtil.FAIL_UPDATE_ENTRY_VIEW));
        }
    }
    ...
    private void storageKnowhowDetailInputFile() throws IOException {
        ...
        String workDir = fileInput.getFile().getProject().getName()
                + ApplicationPropertiesUtil
                        .getProperty(ApplicationPropertiesUtil.WORK_DIRECTORY);
        FileUtil.createWorkDir(container, workDir);
        knowhowDetailTempFile = workDir
                + ApplicationPropertiesUtil
                        .getProperty(ApplicationPropertiesUtil.DOCBOOK_TEMPORARY_PATH);
    }
     ...
     *             IO exception
     */
    private void createDocBookPage(boolean readOnly) throws PartInitException,
            IOException {
    ...
    public void doSave(IProgressMonitor monitor) {
        ...
        if (!PluginUtil.isTreeViewerDisposed()) {
            ...
            doSaveCheck();
            try {
                KnowhowManagement.unCheckWrite(fileLocationFullPath, PluginUtil
                        .getKnowhowEntryViewTreeViewer().getInputEntry(),
                        documentationFormPage.getTreeViewer().getInputEntry());
                super.commitPages(true);
                MaintenanceKnowhowMultiPageEditor.LOGGER
            ...
            openErrorDialog(
                    MessagePropertiesUtil
                            .getMessage(MessagePropertiesUtil.OPEN_KNOWHOW_ENTRY_VIEW),
                    null);
        }
    }
    ...
    public void updateDocumentationFormPage() throws JbmException {
        MaintenanceKnowhowMultiPageEditor.LOGGER.debug(CmnStringUtil.EMPTY);
        this.getTreeViewerOperator().setEntryListData(
                documentationFormPage.createEntryOperation());
    ...
    public void init(IEditorSite site, IEditorInput input)
            throws PartInitException {
        MaintenanceKnowhowMultiPageEditor.LOGGER.debug("[site]" + site
                + "[input]" + input);
        try {
            MaintenanceKnowhowMultiPageEditor.LOGGER.info(MessagePropertiesUtil
                    .getMessage(MessagePropertiesUtil.LOG_OPEN_EDITOR));
            ...
            MaintenanceKnowhowMultiPageEditor.LOGGER.error(e.getMessage(),
                    e.getCause());
     ...
     *             Jbm exception
     */
    private void setUpEditor(IEditorSite site, IEditorInput input)
            throws JbmException {
        // Check Editor of multiple display
        if (PluginUtil.isMultiOpenKnowhowEditor()) {
            throw new JbmException(
                    MessagePropertiesUtil
                            .getMessage(MessagePropertiesUtil.ERROR_MULTI_OPEN_KNOWHOWEDITOR));
        }
        // Initializing process
        try {
            super.init(site, input);
            super.setPartName(((IFileEditorInput) super.getEditorInput())
                    .getFile().getName());
            storageEditorInputFile();
    ...
    public boolean check() {
        MaintenanceKnowhowMultiPageEditor.LOGGER.debug(CmnStringUtil.EMPTY);
        if (knowhowDetailEditor.getKnowhowDetailType() != null) {
            if (KnowhowManagement.isNestSection(knowhowDetailEditor
                    .getKnowhowDetailType().getDocbookdata())) {
                return false;
            }
        }
        return true;
