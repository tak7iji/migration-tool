...
/**
    ...
    public JbossMigrationConvertTool readToJbossMigrationConvertTool(
        ...
        try {
            ...
            MigrationTargets migrationTargets = null;

            ObjectFactory objectFactory = new ObjectFactory();

            JbossMigrationConvertTool top = objectFactory
                    .createJbossMigrationConvertTool();
            MigrationItems items = objectFactory
            ...
            for(String line : lines){
                ...
                String convertType = parseCsv
                        .get(JbmCsvEnum.TOKEN_INDEX_CHANGE_ITEM.getCode());
            ...
            // Since it is necessary to increment the counter
            // createMigrationTarget (),
            // and not using the extended statement For
            for (int lineCount = 0; lineCount < lineList.size(); lineCount++) {
                List<String> parseCsv = CsvUtil.parseCsv(lineList
                        .get(lineCount));

                String number = parseCsv.get(JbmCsvEnum.TOKEN_INDEX_NO
                        .getCode());
                if (StringUtil.isEmpty(currentNumber)) {
                    migrationItem = createMigrationItem(objectFactory,
                            parseCsv, lineCount + 1);
                    migrationTarget = createMigrationTarget(objectFactory,
                            parseCsv);
                    migrationTargets = objectFactory
                            .createMigrationItemMigrationTargets();
                } else if (number.equals(currentNumber)) {
                    migrationTarget = createMigrationTarget(objectFactory,
                            parseCsv);
                } else {
                    migrationItem.setMigrationTargets(migrationTargets);
                ...
                migrationTargets.getMigrationTarget().add(migrationTarget);
                currentNumber = number;
                count++;
            }
            if (migrationItem != null && migrationTargets != null) {
                migrationItem.setMigrationTargets(migrationTargets);
                itemList.add(migrationItem);
            }
            ...
            throw new JbmException(e, LOGGER, ERROR_LEVEL.ERROR, new String[] {
                    MessageUtil.ERR_CONVERT_NOT_FOUND, filePath });
        } catch (IOException e) {
            throw new JbmException(e, LOGGER, ERROR_LEVEL.ERROR, new String[] {
    ...
    /**
     * Create MigrationTarget than those listing the CSV format.<br/>
     * 
     * @param objectFactory
     *            XmlObjectFactory
     * @param parseCsv
     *            List of data rows CSV1
     * @return MigrationTarget
     */
    private MigrationTarget createMigrationTarget(ObjectFactory objectFactory,
            List<String> parseCsv) {
        MigrationTarget createMigrationTarget = objectFactory
                .createMigrationTarget();

        String filePath = parseCsv.get(JbmCsvEnum.TOKEN_INDEX_TARGET_FILE_PATH
                .getCode());
        String targetLine = parseCsv.get(JbmCsvEnum.TOKEN_INDEX_ROW_NO
                .getCode());

        TargetLines createMigrationTargetTargetLines = objectFactory
                .createMigrationTargetTargetLines();
        List<String> targetLineList = createMigrationTargetTargetLines
                .getTargetLine();
        if (!StringUtil.EMPTY.equals(targetLine)) {
            for (String temp : targetLine.split(StringUtil.BLANK)) {
                targetLineList.add(temp);
            }
        }

        createMigrationTarget.setConvert(false);
        createMigrationTarget.setTargetFile(filePath);
        createMigrationTarget.setTargetLines(createMigrationTargetTargetLines);
        createMigrationTarget.setConvertLines(objectFactory
                .createMigrationTargetConvertLines());
        return createMigrationTarget;
    }

    /**
     * Create MigrationItem than those listing the CSV format.<br/>
     * Automatic conversion item format during normal operation and create a
     * MigrationItem.<br/>
     * 
     * @param objectFactory
     *            XmlObjectFactory
     ...
     * @throws JbmException
     *             Format abnormal (automatic conversion of abnormal items JBM
     *             file)
     */
    private MigrationItem createMigrationItem(ObjectFactory objectFactory,
            List<String> parseCsv, int line) throws JbmException {
        String no = parseCsv.get(JbmCsvEnum.TOKEN_INDEX_NO.getCode());
        String convertItem = parseCsv.get(JbmCsvEnum.TOKEN_INDEX_CHANGE_ITEM
                .getCode());
        ConvertType convert = getConvertType(convertItem);
        if (convert == null) {
            throw new JbmException(null, LOGGER, ERROR_LEVEL.ERROR,
                    new String[] { MessageUtil.ERR_CONVERT_TYPE,
                            String.valueOf(line) });
        }

        MigrationItem migrationItem = objectFactory.createMigrationItem();
        migrationItem.setNumber(no);
        migrationItem.setConvertType(convert);
        return migrationItem;
    }

    /**
     * Get automatic conversion item value.<br/>
     * 
     * @param convert
     *            Automatic conversion item value
     * @return Automatic conversion item (@ see ConvertType)
     */
    private ConvertType getConvertType(String convert) {
        for (ConvertType type : ConvertType.values()) {
            if (type.toString().equals(convert)) {
                return type;
            }
        }
        return null;
